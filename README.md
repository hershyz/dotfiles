# dotfiles

Currently just my opinionated nvim config and some iterm2 keymaps.  
_Definitely_ not a template, meant to feel as Mac-native as possible.  
Additional notes/non-nvim (iterm2) configs at the bottom of `init.lua`.

<img width="1743" height="1150" alt="Screenshot 2026-03-05 at 1 11 07 AM" src="https://github.com/user-attachments/assets/425f94c5-7604-4ab0-a6cf-67f255458859" />


#### TODO (in rough order of importance):
- [X] Native-like buffer selection funcionality
- [X] Native-like `cmd + c`, `cmd + x`, `cmd + v` functionality
- [X] Native-like `cmd + [arrow]` functionality
- [X] Git integration (gitsigns)
- [X] Separate keymaps for grep on all files in src dir versus current buffer
- [X] Native-like `cmd + s` functionality
- [X] Multi-cursor for comment toggle and indent right/left on a selected region
- [X] Go to definition keymap
- [X] Automatic file-tree update on git actions (e.g., push)
- [X] Keymap to jump through warnings/errors in current buffer
- [X] `()`, `[]`, and `{}` wrapping a selection within a buffer
- [ ] Distinguished icons between warnings, errors, and hints within a buffer
- [X] Mapping for selection of entire buffer + ability to delete/write over directly afterwards
- [X] Smooth scrolling?
